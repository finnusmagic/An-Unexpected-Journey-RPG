...
Shader "BOXOPHOBIC/LowPolyWater V2/SIMPLE"
{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float2 temp_cast_0 = (ase_screenPos14.w).xx;
			o.data567 = ( ( componentMask15 + componentMask45 ) / temp_cast_0 );
			...
			float4 temp_cast_1 = (max( dot( temp_output_6_0 , ase_lightDir ) , 0.0 )).xxxx;
			float3 ase_worldNormal = UnityObjectToWorldNormal( v.normal );
			float3 ase_indirectDiffuse = ShadeSH9( float4( ase_worldNormal, 1 ) );
			o.data451 = ( ( temp_cast_1 * _LightColor0 ) + float4( ase_indirectDiffuse , 0.0 ) );
			float4 temp_cast_3 = (( pow( max( dot( temp_output_6_0 , normalize( ( ase_lightDir + worldViewDir ) ) ) , 0.0 ) , ( _WaterGloss * 128.0 ) ) * _WaterSpecular )).xxxx;
			float4 temp_output_440_0 = ( temp_cast_3 * _LightColor0 );
			o.data441 = temp_output_440_0;
			float3 temp_cast_4 = (( lerp( ase_worldPos.z , ase_worldPos.x , _WaveDirectionZX ) + ( _Time.x * _WaveSpeed ) )).xxx;
			float3 appendResult159 = float3( 0 , _WaveCycles , 0 );
			float3 temp_cast_5 = (( _WaveHeight * 0.1 )).xxx;
			float3 WAVEMOTION = ( sin( ( temp_cast_4 * appendResult159 ) ) * temp_cast_5 );
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			Input i = s.SurfInput;
			half4 c = 0;
			float4 temp_cast_0 = (max( _AbsorptionIntensity , 1.0 )).xxxx;
			...
			fixed4 REFRACTION = lerp( _WaterColor , ( _AbsorptionColor * temp_cast_0 ) , ( DEPTH * min( _AbsorptionIntensity , 1.0 ) ) );
			fixed4 REFLECTION = tex2D( _ReflectionTex, i.data567 );
			float FRESNEL = i.data606;
			...
			fixed4 LIGHTING_LAMBERT = i.data451;
			float3 ase_worldPos = i.worldPos;
			float3 ase_lightDir = normalize( UnityWorldSpaceLightDir( ase_worldPos ) );
			float4 temp_cast_4 = (( pow( max( dot( temp_output_6_0 , normalize( ( ase_lightDir + i.viewDir ) ) ) , 0.0 ) , ( _WaterGloss * 128.0 ) ) * _WaterSpecular )).xxxx;
			float4 temp_output_440_0 = ( temp_cast_4 * _LightColor0 );
			#ifdef _PERVERTEXSPECULAR_ON
			float4 staticSwitch500 = i.data441;
			#else
			float4 staticSwitch500 = temp_output_440_0;
			#endif
			fixed4 LIGHTING_SPECULAR = staticSwitch500;
			float eyeDepth409 = LinearEyeDepth(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos161))));
			fixed EDGE = ( ( 1.0 - floor( saturate( ( _EdgeOffset + abs( ( eyeDepth409 - ase_screenPos161.w ) ) ) ) ) ) * _EdgeIntensity );
			fixed OPACITY = saturate( ( lerp( ( 1.0 - DEPTH ) , 1.0 , _AbsorptionIntensity ) + EDGE ) );
			c.rgb = lerp( ( saturate( ( ALBEDO * LIGHTING_LAMBERT ) ) + LIGHTING_SPECULAR ) , _EdgeColor , EDGE ).rgb;
			c.a = OPACITY;
			return c;
		}
...
/*ASEBEGIN
Version=10010
1927;29;1906;1014;2628.342;152.3781;1;True;False
Node;AmplifyShaderEditor.Vector3Node;104;-3328,-3440;Fixed;False;Constant;_Vector0;Vector 0;6;0;0,1,0;0;4;FLOAT3;FLOAT;FLOAT;FLOAT
Node;AmplifyShaderEditor.WorldNormalVector;265;-3328,-3584;Float;False;1;0;FLOAT3;0,0,0;False;4;FLOAT3;FLOAT;FLOAT;FLOAT
Node;AmplifyShaderEditor.RangedFloatNode;9;-3328,-3296;Fixed;False;Property;_SmoothNormals;Smooth Normals;6;0;0.5;0;1;0;1;FLOAT
Node;AmplifyShaderEditor.LerpOp;6;-2976,-3584;Float;False;3;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT;0.0;False;1;FLOAT3
Node;AmplifyShaderEditor.RelayNode;587;-2560,-1792;Float;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3
Node;AmplifyShaderEditor.ComponentMaskNode;414;-1920,-1792;Float;False;True;False;True;False;1;0;FLOAT3;0,0,0,0;False;1;FLOAT2
Node;AmplifyShaderEditor.ScreenPosInputsNode;161;-1920,-1664;Float;False;1;False;0;5;FLOAT4;FLOAT;FLOAT;FLOAT;FLOAT
Node;AmplifyShaderEditor.SimpleAddOpNode;415;-1408,-1792;Float;False;2;2;0;FLOAT2;0.0;False;1;FLOAT4;0,0,0,0;False;1;FLOAT4
...
Node;AmplifyShaderEditor.RelayNode;608;-2560,-64;Float;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.WorldSpaceLightDirHlpNode;446;-1920,-3520;Float;False;1;0;FLOAT;0.0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.SimpleDivideOpNode;20;-1328,-160;Float;False;2;0;FLOAT2;0.0,0;False;1;FLOAT;0.0;False;1;FLOAT2
Node;AmplifyShaderEditor.RelayNode;582;-2560,-3200;Float;False;1;0;FLOAT3;0.0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.RelayNode;630;-2560,-3584;Float;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.AbsOpNode;605;-1392,-2304;Float;False;1;0;FLOAT;0.0;False;1;FLOAT
...
Node;AmplifyShaderEditor.LerpOp;362;-1024,256;Float;False;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0.0;False;1;COLOR
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;445;-1216,-3584;Float;False;2;2;0;FLOAT;0.0;False;1;COLOR;0,0,0,0;False;1;COLOR
...
Node;AmplifyShaderEditor.IndirectDiffuseLighting;468;-1344,-3440;Float;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.RangedFloatNode;80;-1920,1568;Float;False;Property;_WaveSpeed;Wave Speed;17;0;25;0;0;0;1;FLOAT
...
Node;AmplifyShaderEditor.LerpOp;146;-1536,-624;Float;False;3;0;COLOR;0.0,0,0,0;False;1;FLOAT4;0.0,0,0,0;False;2;FLOAT;0.0;False;1;FLOAT4
...
Node;AmplifyShaderEditor.VertexToFragmentNode;451;-464,-3584;Float;False;1;0;COLOR;0.0;False;1;COLOR
...
Node;AmplifyShaderEditor.StaticSwitch;556;-1280,-768;Float;False;Property;_DisableReflection;Disable Reflection;1;0;0;False;2;0;COLOR;0.0;False;1;COLOR;0.0,0,0,0;False;1;COLOR
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;440;-704,-3200;Float;False;2;2;0;FLOAT;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR
Node;AmplifyShaderEditor.RangedFloatNode;160;-1664,1568;Float;False;Property;_WaveCycles;Wave Cycles;16;0;1.5;0;0;0;1;FLOAT
...
Node;AmplifyShaderEditor.VertexToFragmentNode;441;-464,-3200;Float;False;1;0;COLOR;0.0;False;1;COLOR
Node;AmplifyShaderEditor.AppendNode;159;-1440,1504;Float;False;FLOAT3;0;0;0;0;4;0;FLOAT;0.0;False;1;FLOAT;0.0;False;2;FLOAT;0.0;False;3;FLOAT;0.0;False;1;FLOAT3
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;450;128,-3584;Fixed;False;LIGHTING_LAMBERT;-1;True;1;0;COLOR;0,0,0,0;False;1;COLOR
Node;AmplifyShaderEditor.GetLocalVarNode;622;0,384;Float;False;246;0;1;FLOAT
Node;AmplifyShaderEditor.RangedFloatNode;56;-1280,1568;Float;False;Property;_WaveHeight;Wave Height;15;0;0.5;0;0;0;1;FLOAT
...
Node;AmplifyShaderEditor.StaticSwitch;500;-256,-3120;Float;False;Property;_PerVertexSpecular;Per Vertex Specular;0;0;0;False;2;0;COLOR;0.0;False;1;COLOR;0.0,0,0,0;False;1;COLOR
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;455;128,-3200;Fixed;False;LIGHTING_SPECULAR;-1;True;1;0;COLOR;0,0,0,0;False;1;COLOR
...
Node;AmplifyShaderEditor.LerpOp;492;-896,-4480;Float;False;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0.0;False;1;COLOR
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;98;-704,1328;Float;False;WAVEMOTION;-1;True;1;0;FLOAT3;0.0;False;1;FLOAT3
Node;AmplifyShaderEditor.StandardSurfaceOutputNode;0;-512,-4480;Float;False;True;2;Float;ASEMaterialInspector;200;CustomLighting;BOXOPHOBIC/LowPolyWater V2/SIMPLE;False;False;False;False;True;True;True;True;True;False;True;True;False;False;True;True;Back;0;0;False;0;0;Transparent;0.5;True;False;0;False;Transparent;Transparent;All;True;True;True;True;True;True;True;True;True;True;True;True;True;True;True;True;True;False;0;255;255;0;0;0;0;False;0;4;10;25;False;0.5;False;0;SrcAlpha;OneMinusSrcAlpha;0;SrcAlpha;OneMinusSrcAlpha;OFF;Add;0;False;0;0,0,0,0;VertexOffset;False;Cylindrical;Relative;200;Unlit/Color;-1;-1;-1;-1;14;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT;0.0;False;4;FLOAT;0.0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT;0.0;False;9;FLOAT;0.0;False;10;OBJECT;0.0;False;11;FLOAT3;0,0,0;False;12;FLOAT3;0,0,0;False;14;FLOAT4;0,0,0,0;False;15;FLOAT3;0,0,0;False;0
Node;AmplifyShaderEditor.CommentaryNode;272;-2176,-768;Float;False;100;100;;0;// ALBEDO;0;0
Node;AmplifyShaderEditor.CommentaryNode;276;-2176,-256;Float;False;100;100;;0;// REFLECTION MIRROR;0;0
Node;AmplifyShaderEditor.CommentaryNode;303;-2176,-2304;Float;False;100;100;;0;// FRESNEL CONTROL;0;0
Node;AmplifyShaderEditor.CommentaryNode;277;-2176,256;Float;False;100;100;;0;// REFRECTION;0;0
Node;AmplifyShaderEditor.CommentaryNode;280;-2176,-4480;Float;False;100;100;;0;// FINAL;0;0
Node;AmplifyShaderEditor.CommentaryNode;274;-2176,-1792;Float;False;100;100;;0;// DEPTH AND EDGE CONTROL;0;0
Node;AmplifyShaderEditor.CommentaryNode;458;-2176,-3200;Float;False;100;100;;0;// LIGHTING SPECULAR;0;0
Node;AmplifyShaderEditor.CommentaryNode;510;-2176,-3584;Float;False;100;100;;0;// LIGHTING LAMBERT;0;0
Node;AmplifyShaderEditor.CommentaryNode;457;-3584,-3584;Float;False;100;100;;0;// NORMALS;0;0
Node;AmplifyShaderEditor.CommentaryNode;279;-2176,1152;Float;False;100;100;;0;// WAVE MOTION;0;0
...
WireConnection;608;0;6;0
WireConnection;15;0;14;0
WireConnection;45;0;608;0
...
ASEEND*/
//CHKSM=949C89B710C8B41FB1A416BD7F15C45C07DC63AC